...
for(x in exp_list){
  for(y in out_list){
      df_raw <- fread(paste0(datadir,resultsdir,x, "/", y, "_", x, "_PRS_results_lm.csv")) %>% select(N, score, beta, Upper_CI, Lower_CI)
      
      df <- fread(paste0(datadir,resultsdir, x, "/", y, "_", x, "_PRS_MSOA_results_Mundlak_risk_score.csv")) %>% 
      select(N, score, beta, Upper_CI, Lower_CI)
      
      df_mean <- fread(paste0(datadir,resultsdir, x, "/", y, "_", x, "_PRS_MSOA_results_Mundlak_risk_score_mean.csv")) %>% 
      select(N, score, beta, Upper_CI, Lower_CI)
